...
/*
Chương trình đọc vào 1 văn bản từ file, in ra các từ viết hoa, các tên riêng và tần suất xuất hiện của các từ.
*/
...
void CleanParagraph(FILE *fIN1) // Làm sạch văn bản, thay các dấu . , ! ? thành dấu cách
{
      ...
      else
      {
            ...
            {
                  if (c == 33 || c == 34 || c == 39 || c == 44 || c == 46 || c == 63)
                  {
                        c = 32;
                        fputc(c, fOUT1);
                  }
...
void NumberOfOccurence(int num_satisfied_word, int num_word, char **SatisfiedWords, char **BagOfWords) //Đếm số từ xuất hiện và in ra từ và tần số xuất hiện.
{
      ...
      for (int i = 0; i < num_satisfied_word - 1; i++)
      {
            for (int j = i; j < num_satisfied_word; j++)
            {
                  if (occurence[i] < occurence[j])
                  {
                        ...
                        occurence[j] = temp;
                        strcpy(tempWord, SatisfiedWords[i]);
                        strcpy(SatisfiedWords[i], SatisfiedWords[j]);
                        strcpy(SatisfiedWords[j], tempWord);
                  }
            }
      }
